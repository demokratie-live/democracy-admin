...
const postHC = ({procedureId} : {procedureId:string}) => () => {
  ...
  http.get(`http://localhost:3000/webhooks/human-connection/contribute?procedureId=${procedureId}`, (resp) => {
    console.log(resp);
  });
}
...
export const ProcedureList: React.SFC<IProps> = props => (
  <List>
    <ProcedureListQuery query={proceduresListQuery} variables={{listTypes: props.listTypes}}>
      {({ data }: IProcedureListProps) => {
        return (!data || !data.procedures) ? null : data.procedures.map(({ title, procedureId }) => {
          return <List.Item key={procedureId}><p>{title}</p><button onClick={postHC({procedureId})}>Post to HC</button></List.Item>;
        });
      }}
    </ProcedureListQuery>
  </List>
